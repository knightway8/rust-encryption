...
use otp1::{EncryptOutcome, encrypt_in_place, key_path_next_to_current_exe};

const USAGE: &str = "Usage: otp1 [--] <input-file>\n\
\n\
Atomically XOR each input byte with the corresponding byte from key.key,\n\
which must be beside the otp1 executable. Applying the same key twice restores\n\
...
fn main() -> ExitCode {
    match parse_input_path(env::args_os().skip(1)) {
        ParseResult::Help => match io::stdout().lock().write_all(USAGE.as_bytes()) {
            Ok(()) => ExitCode::SUCCESS,
            Err(error) => {
                report_stderr(&format!("otp1: cannot write help output: {error}"));
                ExitCode::from(1)
            }
        },
        ParseResult::UsageError(message) => {
            report_stderr(&format!("otp1: {message}\n\n{USAGE}"));
            ExitCode::from(2)
        }
        ParseResult::Input(input_path) => {
            let key_path = match key_path_next_to_current_exe() {
                Ok(path) => path,
                Err(error) => {
                    report_stderr(&format!("otp1: {error}"));
                    return ExitCode::from(1);
                }
            };

            match encrypt_in_place(&input_path, &key_path) {
                Ok(EncryptOutcome::Committed) => ExitCode::SUCCESS,
                Ok(EncryptOutcome::CommittedButDurabilityUncertain(error)) => {
                    report_stderr(&format!(
                        "otp1: the file was atomically transformed, but crash durability could \
                         not be confirmed: {error}. DO NOT RETRY automatically; another XOR \
