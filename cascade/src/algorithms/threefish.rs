//! Threefish-1024-CBC with PKCS#7 padding and encrypt-then-MAC authentication.
//!
...
/// Threefish-1024's block and CBC initialization-vector size, in bytes.
pub const IV_LEN: usize = 128;
/// Threefish-1024's block size, in bytes.
pub const BLOCK_LEN: usize = 128;

const MASTER_KEY_LEN: usize = 128;
const ENCRYPTION_KEY_LEN: usize = 128;
...
const KDF_ENCRYPTION_INFO: &[u8] = b"cascade:file:v1:threefish-1024-cbc:encryption-key";
const KDF_AUTHENTICATION_INFO: &[u8] = b"cascade:file:v1:threefish-1024-cbc:hmac-sha512-key";
const MAC_DOMAIN: &[u8] = b"cascade:file:v1:threefish-1024-cbc:hmac-sha512\0";

type CbcEncryptor = cbc::Encryptor<Threefish1024>;
type CbcDecryptor = cbc::Decryptor<Threefish1024>;
...
pub(super) fn seal(
    ...
    let cipher = <Threefish1024 as KeyInit>::new_from_slice(keys.encryption.as_ref())
...
mod tests {
    ...
    #[test]
    fn threefish_1024_matches_skein_known_answer() {
        // Skein/Threefish reference vector: fixed zero tweak, all-zero key,
        // and all-zero plaintext block.
        let cipher = Threefish1024::new_from_slice(&[0_u8; MASTER_KEY_LEN]).unwrap();
        let mut block = Block::<Threefish1024>::default();
    ...
    fn header_ciphertext_tag_and_salt_tampering_are_rejected() {
        let (ciphertext, tag) = seal(&KEY, &SALT, &IV, HEADER, &plaintext(256)).unwrap();
    ...
    fn authenticated_malformed_padding_is_rejected() {
        // A full padding block follows this 128-byte plaintext. Flipping the
    ...
    fn derived_key_roles_are_domain_separated() {
        let keys = derive_keys(&KEY, &SALT).unwrap();
        assert_ne!(
            &keys.encryption[..AUTHENTICATION_KEY_LEN],
