...
impl Fixture {
    fn new(label: &str, input: &[u8], key: Option<&[u8]>) -> io::Result<Self> {
        ...
        let executable = directory.path().join("otp2-under-test");
        ...
        fs::copy(env!("CARGO_BIN_EXE_otp2"), &executable)?;
    ...
    #[cfg(target_os = "linux")]
    fn spawn_with_umask(&self, mask: libc::mode_t) -> io::Result<Child> {
        ...
        for attempt in 0..100 {
            ...
            // `umask` system call before exec and does not access shared state.
            unsafe {
    ...
    loop {
        ...
        if started.elapsed() >= timeout {
            ...
            return Err(io::Error::new(
                ...
                "otp2 did not exit before the test timeout",
            ));
        }
        thread::yield_now();
    }
...
fn visible_otp_temp(directory: &Path) -> io::Result<Option<PathBuf>> {
    for entry in fs::read_dir(directory)? {
        ...
        if name.starts_with(".otp2-") && name.ends_with(".tmp") {
    ...
    loop {
        ...
        if let Some(status) = child.try_wait()? {
            return Err(io::Error::other(format!(
                "otp2 exited with {status} before creating its temporary output"
            )));
        }
        if started.elapsed() >= Duration::from_secs(5) {
            ...
            return Err(io::Error::new(
                io::ErrorKind::TimedOut,
                "otp2 did not expose its temporary output before the timeout",
            ));
        }
        thread::yield_now();
    }
...
fn stop_child(child: &mut Child) -> io::Result<()> {
    ...
    if waited != child.id() as i32 || status & 0xff != 0x7f {
        Err(io::Error::other(
            "otp2 exited before the stop signal took effect",
        ))
    } else {
        Ok(())
    }
...
#[test]
fn atomic_replace_preserves_a_read_only_linux_mode() -> io::Result<()> {
    ...
    assert_eq!(
        ...
        original_mode,
        "replacement changed the input's Linux mode"
    );
        ...
        loop {
            ...
            if report.observations == 1 {
                if snapshot != Snapshot::Old {
                    let error = "input was not old before otp2 started".to_owned();
                    let _ = ready_sender.send(Err(error.clone()));
                    return Err(error);
                }
                ready_sender
...
fn guessed_temp_name_files_and_symlinks_are_never_overwritten() -> io::Result<()> {
    ...
    let symlink_candidate = fixture.directory.path().join(".otp2.tmp");
...
#[cfg(target_os = "linux")]
#[test]
fn retargeting_a_symlinked_parent_cannot_redirect_an_in_flight_transform() -> io::Result<()> {
    const FILE_SIZE: usize = 32 * 1024 * 1024;
    const PLAINTEXT: u8 = 0x39;
    const KEY: u8 = 0xa6;
    const ALTERNATE: &[u8] = b"the retargeted directory must remain untouched";

    let _serial = BARRIER_TEST_LOCK
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let mut fixture = Fixture::new("parent-symlink-retarget", &[], Some(&[]))?;
    let real_directory = fixture.directory.path().join("original data directory");
    let anchored_directory = fixture.directory.path().join("renamed anchored directory");
    let alternate_directory = fixture.directory.path().join("alternate data directory");
    let parent_alias = fixture.directory.path().join("current data directory");
    fs::create_dir(&real_directory)?;
    fs::create_dir(&alternate_directory)?;

    let real_input = real_directory.join("payload.bin");
    let alternate_input = alternate_directory.join("payload.bin");
    write_repeated(&real_input, PLAINTEXT, FILE_SIZE)?;
    write_repeated(&fixture.key, KEY, FILE_SIZE)?;
    fs::write(&alternate_input, ALTERNATE)?;
    unix_fs::symlink(&real_directory, &parent_alias)?;
    fixture.input = parent_alias.join("payload.bin");

    let mut child = fixture.spawn()?;
    let temporary = wait_for_otp_temp(&mut child, &real_directory)?;
    stop_child(&mut child)?;
    assert!(
        fs::metadata(&temporary)?.len() < FILE_SIZE as u64,
        "the parent-retarget barrier was reached after the transform completed"
    );

    fs::rename(&real_directory, &anchored_directory)?;
    fs::remove_file(&parent_alias)?;
    unix_fs::symlink(&alternate_directory, &parent_alias)?;
    continue_child(&child)?;
    let output = wait_for_exit(child, Duration::from_secs(30))?;

    assert_success(&output);
    let anchored_input = anchored_directory.join("payload.bin");
    assert_file_is_byte(&anchored_input, PLAINTEXT ^ KEY, FILE_SIZE)?;
    assert_eq!(fs::read(&alternate_input)?, ALTERNATE);
    assert_eq!(fs::read_link(&parent_alias)?, alternate_directory);
    assert!(
        visible_otp_temp(&anchored_directory)?.is_none(),
        "successful anchored transform left a sibling temporary file"
    );
    Ok(())
}
...
fn forced_termination_before_rename_leaves_original_inode_and_restrictive_temp() -> io::Result<()> {
    ...
    let mut child = fixture.spawn_with_umask(0)?;
    let temporary = wait_for_otp_temp(&mut child, fixture.directory.path())?;
    stop_child(&mut child)?;
    ...
    assert_eq!(temporary_metadata.mode() & 0o777, 0o600);
    child.kill()?;
