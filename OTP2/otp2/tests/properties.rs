...
fn transform(input: &[u8], key: &[u8], length: u64) -> io::Result<Vec<u8>> {
    ...
    otp2::xor_stream_exact(&mut input, &mut key, &mut output, length)?;
    Ok(output)
}
...
fn zero_length_does_not_access_any_stream() {
    ...
    otp2::xor_stream_exact(&mut input, &mut key, &mut output, 0).unwrap();
}
...
fn output_is_appended_at_the_writer_position() {
    ...
    otp2::xor_stream_exact(&mut input, &mut key, &mut output, 2).unwrap();
...
fn processes_exactly_the_requested_prefix() {
    ...
    otp2::xor_stream_exact(&mut input, &mut key, &mut output, 4).unwrap();
...
fn key_may_be_longer_than_input() {
    ...
    otp2::xor_stream_exact(&mut input, &mut key, &mut output, 3).unwrap();
...
fn one_byte_partial_readers_are_supported() {
    ...
    otp2::xor_stream_exact(&mut input, &mut key, &mut output, expected.len() as u64).unwrap();
...
fn premature_input_eof_is_reported() {
    ...
    let error = otp2::xor_stream_exact(&mut input, &mut key, &mut output, 4).unwrap_err();
...
fn immediate_input_eof_is_reported() {
    ...
    let error = otp2::xor_stream_exact(&mut input, &mut key, &mut output, 1).unwrap_err();
...
fn a_zero_read_before_requested_length_is_eof() {
    ...
    let error = otp2::xor_stream_exact(&mut input, &mut key, &mut output, 32).unwrap_err();
...
fn input_read_errors_are_propagated() {
    ...
    let error = otp2::xor_stream_exact(&mut input, &mut key, &mut output, 100).unwrap_err();
...
fn writer_errors_are_propagated_without_corrupting_the_prefix() {
    ...
    let error = otp2::xor_stream_exact(&mut input, &mut key, &mut output, expected.len() as u64)
        .unwrap_err();
...
fn a_zero_length_write_is_reported_as_write_zero() {
    ...
    let error = otp2::xor_stream_exact(&mut input, &mut key, &mut output, 3).unwrap_err();
...
fn virtual_stream_uses_bounded_chunks_and_exact_counts_without_storing_output() {
    ...
    otp2::xor_stream_exact(&mut input, &mut key, &mut output, LENGTH).unwrap();
...
fn late_input_error_after_three_chunks_preserves_exact_completed_output_count() {
    ...
    let error = otp2::xor_stream_exact(&mut input, &mut key, &mut output, LENGTH).unwrap_err();
