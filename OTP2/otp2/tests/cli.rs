...
impl Fixture {
    fn new(label: &str) -> Self {
        ...
        let source = PathBuf::from(env!("CARGO_BIN_EXE_otp2"));
    ...
    fn write_key(&self, bytes: &[u8]) {
        ...
        fs::write(&path, bytes).expect("write adjacent key file");
        #[cfg(target_os = "linux")]
        {
            use std::os::unix::fs::PermissionsExt;
            fs::set_permissions(&path, fs::Permissions::from_mode(0o600))
                .expect("make adjacent key private");
        }
    }
        ...
        S: AsRef<OsStr>,
    {
        ...
        for attempt in 0..100 {
            match self.command().args(&args).output() {
                ...
                Err(error) => panic!("launch isolated otp2 executable: {error}"),
            }
        }
...
#[cfg(target_os = "linux")]
#[test]
fn successful_sparse_file_round_trip_preserves_every_logical_byte() {
    use std::fs::OpenOptions;
    use std::io::{Seek, SeekFrom, Write};

    let fixture = Fixture::new("sparse-round-trip");
    let input = fixture.input_path("sparse.bin");
    ...
    file.set_len(length as u64).expect("set sparse length");
    for (offset, island) in [
        (0_u64, b"start".as_slice()),
        (65_535, b"boundary-island".as_slice()),
        (1_500_003, b"late island".as_slice()),
        ((length - 3) as u64, b"end".as_slice()),
    ] {
        file.seek(SeekFrom::Start(offset))
            .expect("seek sparse input");
        file.write_all(island).expect("write sparse island");
    }
    drop(file);

    let original = fs::read(&input).expect("read sparse logical bytes");
    let key = pseudo_random_bytes(length + 31, 0x19aa_72cc_5510_0042);
    fixture.write_key(&key);

    assert_success_quiet(&fixture.run([input.as_os_str()]));
...
#[test]
fn encrypted_bytes_can_be_copied_then_decrypted_in_another_directory() {
    let fixture = Fixture::new("relocated-ciphertext");
    let original = pseudo_random_bytes(131_091, 0xa1b2_c3d4_e5f6_7788);
    let key = pseudo_random_bytes(original.len() + 19, 0x1020_3040_5060_7080);
    let source = fixture.write_input("source/plain.bin", &original);
    fixture.write_key(&key);

    assert_success_quiet(&fixture.run([source.as_os_str()]));
    let ciphertext = fs::read(&source).expect("read encrypted source");
    assert_ne!(ciphertext, original);

    let copied = fixture.input_path("downloaded/renamed-object.bin");
    fs::create_dir_all(copied.parent().unwrap()).expect("create download directory");
    fs::copy(&source, &copied).expect("copy encrypted bytes");
    assert_success_quiet(&fixture.run([copied.as_os_str()]));
...
#[cfg(target_os = "linux")]
#[test]
fn linux_character_devices_are_rejected_without_blocking() {
    let fixture = Fixture::new("character-devices");
    fixture.write_key(&vec![0x42; 4096]);

    for device in ["/dev/null", "/dev/zero"] {
        if !Path::new(device).exists() {
            continue;
        }
        let output = fixture.run([device]);
        assert_runtime_error(&output);
        assert!(
            String::from_utf8_lossy(&output.stderr).contains("regular file"),
            "device rejection was not actionable for {device}: {:?}",
            String::from_utf8_lossy(&output.stderr)
        );
    }
}
...
#[cfg(target_os = "linux")]
#[test]
fn dangling_input_and_key_symlinks_are_rejected_non_destructively() {
    use std::os::unix::fs::symlink;

    let input_fixture = Fixture::new("dangling-input-link");
    let missing_input_target = input_fixture.input_path("missing-target");
    let input_link = input_fixture.input_path("dangling-input");
    symlink(&missing_input_target, &input_link).expect("create dangling input symlink");
    input_fixture.write_key(&[0x71; 128]);

    assert_runtime_error(&input_fixture.run([input_link.as_os_str()]));
    assert!(
    ...
    assert_eq!(fs::read_link(&input_link).unwrap(), missing_input_target);

    let key_fixture = Fixture::new("dangling-key-link");
    let original = b"input must remain unchanged";
    let input = key_fixture.write_input("payload.bin", original);
    let missing_key_target = key_fixture.bin_dir.join("missing-key-target");
    symlink(&missing_key_target, key_fixture.key_path()).expect("create dangling key symlink");

    assert_runtime_error(&key_fixture.run([input.as_os_str()]));
    assert_eq!(fs::read(input).unwrap(), original);
    assert!(
        fs::symlink_metadata(key_fixture.key_path())
            .unwrap()
            .file_type()
            .is_symlink()
    );
    assert_eq!(
        fs::read_link(key_fixture.key_path()).unwrap(),
        missing_key_target
    );
}
...
fn short_key_diagnostic_reports_exact_sizes_without_disclosing_key_material() {
    const SECRET_CANARY: &[u8] = b"OTP2_SECRET_KEY_CANARY_7b4f_DO_NOT_PRINT";
    ...
    for secret_fragment in [
        SECRET_CANARY,
        b"OTP2_SECRET_KEY_CANARY".as_slice(),
...
#[cfg(target_os = "linux")]
#[test]
fn group_or_other_accessible_raw_keys_are_rejected_without_any_changes() {
    use std::os::unix::fs::{MetadataExt, PermissionsExt};

    for mode in [0o640, 0o604, 0o610, 0o601] {
        let fixture = Fixture::new(&format!("public-raw-key-{mode:o}"));
        let original = b"plaintext must survive an unsafe key policy failure";
        let key = pseudo_random_bytes(original.len() + 17, u64::from(mode));
        let input = fixture.write_input("payload.bin", original);
        fixture.write_key(&key);
        fs::set_permissions(fixture.key_path(), fs::Permissions::from_mode(mode))
            .expect("make raw key accessible to group or other users");

        let input_before = fs::metadata(&input).expect("inspect input before rejection");
        let key_before = fs::metadata(fixture.key_path()).expect("inspect key before rejection");
        ...
        assert!(
            stderr.contains("key") && stderr.contains("group or other"),
            "unsafe-key diagnostic was not actionable for mode {mode:o}: {stderr}"
        );
        assert_eq!(fs::read(&input).unwrap(), original);
        assert_eq!(fs::read(fixture.key_path()).unwrap(), key);

        let input_after = fs::metadata(&input).expect("inspect input after rejection");
        let key_after = fs::metadata(fixture.key_path()).expect("inspect key after rejection");
        assert_eq!(
            (input_after.dev(), input_after.ino(), input_after.mode()),
            (input_before.dev(), input_before.ino(), input_before.mode()),
            "unsafe key mode {mode:o} caused the input path or mode to change"
        );
        assert_eq!(
            (input_after.mtime(), input_after.mtime_nsec()),
            (input_before.mtime(), input_before.mtime_nsec()),
            "unsafe key mode {mode:o} caused the input timestamp to change"
        );
        assert_eq!(
            (key_after.dev(), key_after.ino(), key_after.mode()),
            (key_before.dev(), key_before.ino(), key_before.mode()),
            "unsafe key mode {mode:o} caused the key path or mode to change"
        );
...
fn unknown_option_is_a_usage_error() {
    ...
    let output = fixture.run(["--definitely-not-an-otp2-option"]);
    assert_usage_error(&output);
}
...
fn success_does_not_touch_neighboring_or_temp_like_files() {
    ...
    let neighbors = [
        ...
        (".otp2.tmp", b"not a temp file".as_slice()),
        ("payload.bin.tmp", b"also not a temp file".as_slice()),
        (
            ".payload.bin.otp2.tmp",
    ...
    assert_eq!(
        fs::read(fixture.input_path(".otp2.tmp")).unwrap(),
        b"not a temp file"
    );
    assert_eq!(
    ...
    assert_eq!(
        fs::read(fixture.input_path(".payload.bin.otp2.tmp")).unwrap(),
...
#[cfg(target_os = "linux")]
#[test]
fn preserves_multiple_common_linux_permission_modes() {
    use std::os::unix::fs::{MetadataExt, PermissionsExt};
...
#[cfg(target_os = "linux")]
#[test]
fn short_key_preserves_linux_inode_mode_and_modification_time() {
    use std::os::unix::fs::{MetadataExt, PermissionsExt};
