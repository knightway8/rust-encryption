...
const APP_ID: u8 = 18;
const SUITE_NAME: &str = "PBKDF2-SHA512-NORX64-4-1-1048576-records";
const PAYLOAD_ALGORITHM: &str = "NORX64-4-1";
...
fn derive_key(password: &str, salt: &[u8; SALT_SIZE]) -> AppResult<Vec<u8>> {
    ...
    let rounds = if cfg!(test) { 1 } else { 160_000 };
    pbkdf2::pbkdf2_hmac::<sha2::Sha512>(password.as_bytes(), salt, rounds, &mut key);
    Ok(key)
}
...
fn seal_record(
    ...
    let mut output = vec![0_u8; message.len() + TAG_SIZE];
...
fn open_record(
    ...
    let full_len = plaintext_len / norx::constant::BLOCK_LENGTH * norx::constant::BLOCK_LENGTH;
